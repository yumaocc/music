...
import { memo } from 'react';
 ...
 function HistoryList({ state }) {
  ...
  const handleClick = (item ,index) => {
    ...
    dispatch(changePlayList(state))
    dispatch(changeFullScreen(true))
  }
  state = state.map(item => item.song)
  function rowRenderer({
    ...
    return (
      <Item key={key} style={style} onClick={() => handleClick(state[index] ,index)}>
        <div className='index'>{index +1}</div>
        <div className='author'>
          <h1>{state[index].name}</h1>
          <h3>{getName(state[index].ar)} - {state[index].al.name}</h3>
        </div>
        <div className='count'>
